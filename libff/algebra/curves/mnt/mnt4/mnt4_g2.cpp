...
mnt4_Fq2 mnt4_G2::mul_by_a(const mnt4_Fq2 &elt) {
  return mnt4_Fq2(mnt4_twist_mul_by_a_c0 * elt.c0,
                  mnt4_twist_mul_by_a_c1 * elt.c1);
}

mnt4_Fq2 mnt4_G2::mul_by_b(const mnt4_Fq2 &elt) {
  return mnt4_Fq2(mnt4_twist_mul_by_b_c0 * elt.c1,
...
void mnt4_G2::print() const {
  if (this->is_zero()) {
    ...
    gmp_printf("(%Nd*z + %Nd , %Nd*z + %Nd)\n", copy.X_.c1.as_bigint().data,
               mnt4_Fq::num_limbs, copy.X_.c0.as_bigint().data,
               mnt4_Fq::num_limbs, copy.Y_.c1.as_bigint().data,
               mnt4_Fq::num_limbs, copy.Y_.c0.as_bigint().data,
               mnt4_Fq::num_limbs);
  }
}
...
mnt4_G2 mnt4_G2::operator+(const mnt4_G2 &other) const {
  ...
  */
  ...
  if (X1Z2 == X2Z1 && Y1Z2 == Y2Z1) {
    ...
    const mnt4_Fq2 w =
        mnt4_G2::mul_by_a(ZZ) + (XX + XX + XX);  // w   = a*ZZ + 3*XX
    ...
    const mnt4_Fq2 B =
        ((this->X_) + R).squared() - XX - RR;     // B   = (X1+R)^2 - XX - RR
...
mnt4_G2 mnt4_G2::mixed_add(const mnt4_G2 &other) const {
  ...
  const mnt4_Fq2 &X1Z2 =
      (this->X_);  // X1Z2 = X1*Z2 (but other is special and not zero)
  const mnt4_Fq2 X2Z1 = (this->Z_) * (other.X_);  // X2Z1 = X2*Z1

  // (used both in add and double checks)

  const mnt4_Fq2 &Y1Z2 =
      (this->Y_);  // Y1Z2 = Y1*Z2 (but other is special and not zero)
  const mnt4_Fq2 Y2Z1 = (this->Z_) * (other.Y_);  // Y2Z1 = Y2*Z1
...
mnt4_G2 mnt4_G2::zero() { return G2_zero(); }

mnt4_G2 mnt4_G2::one() { return G2_one(); }
